...
public class DevicesKeepAliveServices {
    ...
    public void initializeKeepAlive(Collection<DevicesCollection> deviceIds) {
        for (DevicesCollection device : deviceIds) {
            try {
                ...
                logger.error("Failed to schedule KeepAlive job for device {} - {}: {}", device.getId(), device.getNetworkIdentification(), e.getMessage(), e);
            }
        }
    }
    ...
    private void scheduleKeepAliveJob(DevicesCollection device) throws SchedulerException {
        ...
        logger.info("Scheduled KeepAlive job for Device: {} Network: {}", device.getId(), device.getNetworkIdentification());
    }
    ...
    private JobDetail buildJobDetail(DevicesCollection device) {
        return JobBuilder.newJob(KeepAliveJob.class).withIdentity("KeepAliveJob_" + device.getId()).usingJobData("deviceId", String.valueOf(device.getId()))
    ...
    private Trigger buildJobTrigger(JobDetail jobDetail) {
        return TriggerBuilder.newTrigger().forJob(jobDetail).withIdentity(jobDetail.getKey().getName() + "_Trigger").withSchedule(SimpleScheduleBuilder.simpleSchedule().withIntervalInSeconds(15) // Intervalo
                                                                                                                                                                                                   // de
                                                                                                                                                                                                   // 15
                                                                                                                                                       // segundos
                                                                                                                                                       .repeatForever())
